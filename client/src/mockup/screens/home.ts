...
export const homeScreen: MockupScreenRenderer = {
  render(root: HTMLElement, state: MockupState, actions: MockupActions): void {
    ...
    const tile = (screen: 'campaign' | 'collection' | 'compete', title: string, art: string, copy: string): HTMLElement => {
      const node = button(title, () => actions.go(screen), { class: `mockup-home-tile mockup-tile-${screen}` })
    ...
    root.append(tiles, el('button', { class: 'mockup-profile', 'aria-label': 'Open profile' }, '•'))
  },
}
